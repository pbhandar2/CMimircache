...
=======
gint64* get_hit_count_phase_cont(reader_t* reader, gint64 current_phase, gint64 num_phases){
...
>>>>>>> e5199168b52f8b28cd23b9442ce1f2eaabde9d20
    if (reader->base->total_num == -1)
        reader->base->total_num = get_num_of_req(reader);
...
=======
double* get_hit_rate_phase_cont(reader_t* reader, gint64 current_phase, gint64 num_phases){
    ...
    guint64* hit_count_array = get_hit_count_phase_cont(reader, current_phase, num_phases);
    double* hit_rate_array = g_new(double, size+3);
    request_per_phase *= (current_phase+1);
